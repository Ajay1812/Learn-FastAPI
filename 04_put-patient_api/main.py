...
def update_patient(patient_id: str, patient_upadte: UpdatePatient):
    data = load_data()
    if not patient_id in data:
        raise HTTPException(status_code=404, detail="Patient not found")
    exisiting_patient_info = data[patient_id]
    updated_patient_info = patient_upadte.model_dump(exclude_unset=True)
